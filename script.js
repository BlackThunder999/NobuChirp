const NobuTok = (() => {
    ...
    // ========== АУТЕНТИФИКАЦИЯ (никнейм + пароль, без email) ==========
    const renderAuth = () => {
        app.innerHTML = `
        ...
        authForm.addEventListener('submit', async (e) => {
            ...
            try {
                // Генерируем email автоматически (пользователь его не видит)
    ...
    const loadProfile = async () => {
        ...
        // Если профиль не найден (на всякий случай), создаём его
        if (!data) {
            ...
            data = { username, display_name: username, bio: '', avatar_url: null };
        }
        currentProfile = data;
    };
    ...
    const renderProfile = async (userId) => {
        ...
        if (!profile) {
            // Автоматически создаём профиль, если его нет (например, для только что зарегистрированного пользователя)
            if (userId === currentUser?.id) {
                await loadProfile(); // создаст профиль
                renderProfile(userId);
                return;
            } else {
