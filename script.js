...
async function loadFeed() {
    ...
    for (let i = 0; i < posts.length; i++) {
        ...
        html += '<div class="post">' +
            ...
            '</div>' +
            '</div>' +
            (p.content !== 'Медиа' ? '<div class="post-content">' + p.content + '</div>' : '') +
            media +
...
async function showComments(cid) {
    ...
    if (comments && comments.length > 0) {
        for (let i = 0; i < comments.length; i++) {
            h += '<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.08);"><strong>' + comments[i].users.nickname + '</strong>: ' + comments[i].content + '</div>';
        }
    } else {
        h += '<p style="color:#71767b">Нет комментариев</p>';
    }
    h += '<textarea id="commentText" placeholder="Ваш комментарий..." style="width:100%;margin-top:10px;padding:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;color:#fff;font-family:inherit;resize:none;"></textarea>';
    h += '<button class="btn" style="margin-top:8px;" onclick="addComment(\'' + cid + '\')">Отправить</button>';
...
async function reportPost(cid) {
    ...
    if (!reason) return;
    await supabase.from('reports').insert([{ reporter_id: currentUser.id, chirp_id: cid, reason: reason }]);
...
async function searchHash() {
    ...
    const { data } = await supabase.from('chirps')
        .select('*, users:user_id(nickname, emoji, is_verified)')
        .ilike('content', '%#' + q + '%')
        .order('created_at', { ascending: false });
    
    if (data && data.length > 0) {
        document.getElementById('searchResults').innerHTML = data.map(function(p) {
            return '<div class="post"><div class="post-content"><strong>' + p.users.nickname + '</strong>: ' + p.content + '</div></div>';
        }).join('');
    } else {
        document.getElementById('searchResults').innerHTML = '<div class="empty">Ничего не найдено</div>';
    }
}
...
async function loadProfile(uid) {
    ...
    document.getElementById('profileCard').innerHTML = '<div class="auth-card" style="text-align:center;">' +
        '<div style="font-size:60px;">' + u.emoji + '</div>' +
        '<h2>' + u.nickname + ' ' + (u.is_verified ? '<span class="verified">✓</span>' : '') + '</h2>' +
        '<p style="color:#71767b">' + (u.bio || 'Нет описания') + '</p>' +
        '</div>';
    ...
    if (posts && posts.length > 0) {
        document.getElementById('profilePosts').innerHTML = posts.map(function(p) {
            return '<div class="post"><div class="post-content">' + p.content + '</div>' +
        ...
        document.getElementById('profilePosts').innerHTML = '<div class="empty">Нет постов</div>';
    }
}
        ...
        if (data && data.length > 0) {
            c.innerHTML = data.map(function(u) {
                return '<div class="admin-item">' +
                    ...
                    '<button class="btn-sm" onclick="verifyUser(\'' + u.id + '\')">Верификация</button>' +
                    '<button class="btn-sm" style="border-color:rgba(244,33,46,0.3);color:#f4212e;" onclick="banUser(\'' + u.id + '\')">Бан</button>' +
                    '</div>';
            }).join('');
        }
    }
    if (section === 'reports') {
        const { data } = await supabase.from('reports').select('*, reporter:reporter_id(nickname), chirps:chirp_id(content)').eq('status', 'pending');
        c.innerHTML = data && data.length > 0 ? data.map(function(r) {
            return '<div class="admin-item">От: ' + r.reporter.nickname + '<br>Пост: ' + r.chirps.content + '<br>Причина: ' + r.reason + '</div>';
        }).join('') : '<div class="empty">Нет жалоб</div>';
    }
    if (section === 'banned') {
        const { data } = await supabase.from('banned_ips').select('*');
        c.innerHTML = data && data.length > 0 ? data.map(function(ip) {
            return '<div class="admin-item">IP: ' + ip.ip_address + '<br>Причина: ' + ip.reason + '</div>';
        }).join('') : '<div class="empty">Нет забаненных IP</div>';
    }
}
...
function massReset() {
    if (confirm('Сбросить ВСЕ сессии? Все пользователи выйдут из аккаунтов.')) {
        localStorage.clear();
        alert('Готово. Все сессии сброшены.');
    }
}
...
function showDoc(type) {
    var docs = {
        terms: '<h2>Условия использования</h2><p>Запрещены оскорбления, спам, контент 18+. Администрация имеет право блокировать пользователей и удалять контент. Пользователь несёт ответственность за свои публикации.</p>',
        privacy: '<h2>Конфиденциальность</h2><p>Мы собираем: email, никнейм, дату рождения, IP-адрес. Данные используются только для работы сервиса. Не передаём третьим лицам. Данные хранятся на серверах Supabase в ЕС.</p>',
        rules: '<h2>Правила сообщества</h2><p>1. Будьте вежливы<br>2. Без спама<br>3. Без контента 18+<br>4. Не выдавайте себя за других<br>5. Уважайте авторские права<br>6. Без дискриминации<br>7. Сообщайте о нарушениях</p>'
    };
