...
var currentSession = null; // хранит { userId, expires }
var adminPass;
var lastPostTime = 0; // timestamp последнего поста для rate limit

(function init() {
  // Скрытый пароль админки
  var p1 = 'N0b';
  var p2 = 'uSp@';
  var p3 = 'ce2024';
  adminPass = p1 + p2 + p3;
  ...
  emailjs.init('gXxOqmU-XXnrVz8FHro2jA');
...
function registerStep1() {
  ...
  var monthDiff = now.getMonth() - birthDateObj.getMonth();
...
function createChirp() {
  ...
  var videoFile = document.getElementById('post-video').files[0];
  ...
  // Фильтр телефонов и email (просто заглушка)
  ...
  uploadPromise.then(function(urls) {
    var imageUrl = urls.image_url;
    var videoUrl = urls.video_url;
...
function renderChirps(chirps, screenId) {
  ...
  for (var i = 0; i < chirps.length; i++) {
    (function(chirp) {
      ...
      var actions = '<div class="chirp-actions">' +
        '<span onclick="likeChirp(\'' + chirp.id + '\', this)">❤️ <span class="like-count">' + (chirp.likes_count || 0) + '</span></span>' +
        '<span onclick="openComments(\'' + chirp.id + '\')">💬 ' + (chirp.comments_count || 0) + '</span>' +
      ...
      // Загрузить состояние лайка текущего пользователя
...
function checkIfLiked(chirpId, element) {
  ...
  .then(function(res) {
    ...
    // Также обновим счетчик
    supabase.from('likes').select('id', { count: 'exact' }).eq('chirp_id', chirpId)
    .then(function(countRes) {
      var countSpan = element.querySelector('.like-count');
      if (countSpan) countSpan.textContent = countRes.count;
    });
  });
}
...
function isFollowing(userId) {
  // Синхронно не получится, но для кнопки мы просто переключим после действия,
  // поэтому начальное состояние можно не вычислять точно, просто предположим.
  return false; // упрощение
}
...
function containsBadWords(text) {
  var badWords = ['бля', 'хуй', 'пизд', 'ебан', 'залуп']; // краткий список
...
// Глобальные функции доступны через window
