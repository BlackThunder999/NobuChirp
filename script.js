const NobuChirp = (() => {
    ...
    const ADMIN_PASSWORD = 'NobuChirpAdmin2024';
    ...
    const updatePostBtn = () => {
        ...
        if (!btn) return;
        btn.disabled = (!input || (input.value.length === 0 && !pendingImage));
    };
    ...
    const setupComposer = () => {
        ...
        if (!input || !counter || !postBtn) return;
        ...
        document.getElementById('imageInput')?.addEventListener('change', (e) => { const file = e.target.files[0]; if (!file) return; pendingImage = file; const reader = new FileReader(); reader.onload = ev => { const preview = document.getElementById('pendingImagePreview'); const container = document.getElementById('pendingImageContainer'); if (preview && container) { preview.src = ev.target.result; container.classList.remove('hidden'); } updatePostBtn(); }; reader.readAsDataURL(file); });
        document.getElementById('removePendingImage')?.addEventListener('click', () => { pendingImage = null; const container = document.getElementById('pendingImageContainer'); if (container) container.classList.add('hidden'); updatePostBtn(); });
        ...
        postBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const lastDate = currentUser.last_post_date;
                ...
                input.value = ''; pendingImage = null; const container = document.getElementById('pendingImageContainer'); if (container) container.classList.add('hidden'); counter.textContent = '0/280';
                loadFeed();
    ...
    const showComments = async (chirpId) => { const { data } = await supabase.from('comments').select('*').eq('chirp_id', chirpId).order('created_at', { ascending: true }); const overlay = document.createElement('div'); overlay.className = 'modal-overlay'; overlay.innerHTML = `<div class="modal-card"><h3>💬 Комментарии</h3><div style="max-height:50vh;overflow-y:auto;margin-bottom:12px" id="commentsList">${data?.map(c => `<div style="padding:8px 0;border-bottom:1px solid var(--border)"><strong>${esc(c.username)}</strong>: ${esc(c.content)}</div>`).join('') || '<p style="color:var(--text-secondary)">Нет комментариев</p>'}</div><input type="text" id="commentInput" class="modal-input" placeholder="Ваш комментарий..."><button class="modal-btn" id="sendCommentBtn">Отправить</button><button class="modal-btn secondary" id="closeCommentsBtn">Закрыть</button></div>`; document.body.appendChild(overlay); document.getElementById('closeCommentsBtn').addEventListener('click', () => overlay.remove()); document.getElementById('sendCommentBtn').addEventListener('click', async () => { const content = document.getElementById('commentInput').value.trim(); if (!content) return; await supabase.from('comments').insert({ chirp_id: chirpId, user_id: currentUser.id, username: currentUser.username, content }); document.getElementById('commentInput').value = ''; }); };

    const showProfile = async (userId) => { const { data: user } = await supabase.from('users').select('*').eq('id', userId).single(); if (!user) return; const { count: chirps } = await supabase.from('chirps').select('*', { count: 'exact' }).eq('user_id', userId); const { count: followers } = await supabase.from('follows').select('*', { count: 'exact' }).eq('following_id', userId); const { count: following } = await supabase.from('follows').select('*', { count: 'exact' }).eq('follower_id', userId); const isOwn = userId === currentUser.id; const { data: followData } = await supabase.from('follows').select('*').eq('follower_id', currentUser.id).eq('following_id', userId).single(); const overlay = document.createElement('div'); overlay.className = 'modal-overlay'; overlay.innerHTML = `<div class="modal-card"><div style="text-align:center"><div class="profile-emoji">${user.avatar_emoji||'👤'}${user.is_verified?'<span class="verified-badge"><i class="fa-solid fa-check"></i></span>':''}</div><div class="profile-name">${esc(user.username)}</div><div class="profile-username">@${esc(user.username)}</div><div class="profile-bio">${esc(user.bio||'')}</div><div class="profile-stats"><div class="stat"><div class="stat-value">${chirps||0}</div><div class="stat-label">чириков</div></div><div class="stat"><div class="stat-value">${followers||0}</div><div class="stat-label">подписчиков</div></div><div class="stat"><div class="stat-value">${following||0}</div><div class="stat-label">подписок</div></div></div><div class="profile-actions">${isOwn?'<button class="modal-btn" id="editProfileBtn">Редактировать</button>':`<button class="modal-btn" id="followBtn">${followData?'Отписаться':'Подписаться'}</button>`}</div></div><button class="modal-btn secondary" id="closeProfileBtn" style="margin-top:10px">Закрыть</button></div>`; document.body.appendChild(overlay); document.getElementById('closeProfileBtn').addEventListener('click', () => overlay.remove()); if (isOwn) document.getElementById('editProfileBtn').addEventListener('click', () => { overlay.remove(); showEditProfile(); }); else document.getElementById('followBtn').addEventListener('click', async () => { if (followData) { await supabase.from('follows').delete().eq('id', followData.id); } else { await supabase.from('follows').insert({ follower_id: currentUser.id, following_id: userId }); } overlay.remove(); showProfile(userId); }); };
    const showEditProfile = () => { const overlay = document.createElement('div'); overlay.className = 'modal-overlay'; overlay.innerHTML = `<div class="modal-card"><h3>Редактировать профиль</h3><input type="text" id="editBio" class="modal-input" placeholder="О себе" value="${esc(currentUser.bio||'')}"><p style="color:var(--text-secondary);text-align:center;margin:10px 0">Эмодзи:</p><div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center">${['👤','😀','😎','🤖','👽','🦊','🐼','🎃','💎','🔥'].map(e => `<span style="font-size:2rem;cursor:pointer" class="emoji-opt">${e}</span>`).join('')}</div><button class="modal-btn" id="saveProfileBtn">Сохранить</button><button class="modal-btn secondary" id="closeEditBtn">Отмена</button></div>`; document.body.appendChild(overlay); document.getElementById('closeEditBtn').addEventListener('click', () => overlay.remove()); document.getElementById('saveProfileBtn').addEventListener('click', async () => { const bio = document.getElementById('editBio').value.trim(); await supabase.from('users').update({ bio }).eq('id', currentUser.id); currentUser.bio = bio; localStorage.setItem('nobu_user', JSON.stringify(currentUser)); overlay.remove(); }); overlay.querySelectorAll('.emoji-opt').forEach(el => el.addEventListener('click', async () => { await supabase.from('users').update({ avatar_emoji: el.textContent }).eq('id', currentUser.id); currentUser.avatar_emoji = el.textContent; localStorage.setItem('nobu_user', JSON.stringify(currentUser)); overlay.remove(); })); };
    ...
    const showAdminPanel = () => { const overlay = document.createElement('div'); overlay.className = 'modal-overlay'; overlay.innerHTML = `<div class="modal-card" style="max-height:85vh;overflow-y:auto"><h3>🛡️ Админ</h3><h4>🔨 Бан</h4><input type="text" id="banUsername" class="modal-input" placeholder="Никнейм"><select id="banDuration" class="modal-input"><option value="10">10 мин</option><option value="60">1 час</option><option value="360">6 ч</option><option value="1440">24 ч</option><option value="0">Навсегда</option></select><input type="text" id="banReason" class="modal-input" placeholder="Причина"><button class="modal-btn" id="banUserBtn" style="background:var(--danger)">Заблокировать</button><h4>⚠️ Предупреждение</h4><input type="text" id="warnUsername" class="modal-input" placeholder="Никнейм"><input type="text" id="warnReason" class="modal-input" placeholder="Причина"><button class="modal-btn" id="warnUserBtn" style="background:var(--warning)">Предупредить</button><h4>✅ Верификация</h4><input type="text" id="verifyUsername" class="modal-input" placeholder="Никнейм"><button class="modal-btn" id="verifyUserBtn">Выдать ✅</button><h4>🗑️ Удаление поста</h4><input type="text" id="deleteChirpId" class="modal-input" placeholder="ID чирика"><button class="modal-btn" id="deleteChirpBtn" style="background:var(--danger)">Удалить</button><h4>👤 Профиль</h4><input type="text" id="lookupUsername" class="modal-input" placeholder="Никнейм"><button class="modal-btn" id="lookupUserBtn">Посмотреть</button><h4>🔓 Разбан</h4><input type="text" id="unbanUsername" class="modal-input" placeholder="Никнейм"><button class="modal-btn" id="unbanUserBtn" style="background:var(--success)">Разблокировать</button><button class="modal-btn secondary" id="closeAdminBtn" style="margin-top:12px">Закрыть</button></div>`; document.body.appendChild(overlay); document.getElementById('closeAdminBtn').addEventListener('click', () => overlay.remove()); document.getElementById('banUserBtn').addEventListener('click', async () => { const u = document.getElementById('banUsername').value.trim(), m = parseInt(document.getElementById('banDuration').value), r = document.getElementById('banReason').value.trim()||'нарушение'; if(!u)return; const{data:user}=await supabase.from('users').select('id').eq('username',u).single(); if(!user){alert('Не найден');return;} const expires = m === 0 ? null : new Date(Date.now() + m*60000).toISOString(); await supabase.from('users').update({ is_banned: true, ban_reason: r, ban_expires: expires }).eq('id', user.id); alert(`${u} заблокирован`); }); document.getElementById('warnUserBtn').addEventListener('click', async () => { const u = document.getElementById('warnUsername').value.trim(), r = document.getElementById('warnReason').value.trim()||'нарушение'; if(!u)return; const{data:user}=await supabase.from('users').select('id').eq('username',u).single(); if(!user){alert('Не найден');return;} await supabase.from('warnings').insert({ user_id: user.id, username: u, reason: r }); alert(`${u} получил предупреждение`); }); document.getElementById('verifyUserBtn').addEventListener('click', async () => { const u = document.getElementById('verifyUsername').value.trim(); if(!u)return; const{error}=await supabase.from('users').update({ is_verified: true }).eq('username',u); if(error){alert('Ошибка: '+error.message);return;} alert(`${u} верифицирован ✅`); }); document.getElementById('deleteChirpBtn').addEventListener('click', async () => { const id = document.getElementById('deleteChirpId').value.trim(); if(!id)return; const{error}=await supabase.from('chirps').delete().eq('id', id); if(error){alert('Ошибка: '+error.message);return;} alert('Пост удалён'); }); document.getElementById('lookupUserBtn').addEventListener('click', async () => { const u = document.getElementById('lookupUsername').value.trim(); if(!u)return; const{data:user}=await supabase.from('users').select('*').eq('username',u).single(); if(!user){alert('Не найден');return;} alert(`Профиль ${user.username}:\nID: ${user.id}\nВерифицирован: ${user.is_verified?'Да':'Нет'}\nБан: ${user.is_banned?'Да (до '+new Date(user.ban_expires).toLocaleString()+')':'Нет'}`); }); document.getElementById('unbanUserBtn').addEventListener('click', async () => { const u = document.getElementById('unbanUsername').value.trim(); if(!u)return; await supabase.from('users').update({ is_banned: false, ban_reason: null, ban_expires: null }).eq('username', u); alert(`${u} разблокирован`); }); };

    const init = async () => { const saved = localStorage.getItem('nobu_user'); if (saved) { try { currentUser = JSON.parse(saved); const { data: user } = await supabase.from('users').select('*').eq('id', currentUser.id).single(); if (user) { currentUser = user; if (await checkWarningsAndBan(user)) return; renderApp(); } else { renderAuth(); } } catch (e) { localStorage.removeItem('nobu_user'); renderAuth(); } } else { renderAuth(); } window.addEventListener('beforeunload', () => cleanup()); };
    return { init };
})();
