const NobuChirp = (() => {
    ...
    let currentUser = null, pendingImage = null, activeTab = 'feed';
    ...
    // Проверка предупреждений и банов при входе
    const checkWarningsAndBan = async (user) => {
        ...
        // Проверка непрочитанных предупреждений
    ...
    const showBanScreen = (user) => {
        ...
        app.innerHTML = `<div class="auth-container"><div class="auth-card" style="max-width:500px"><div style="font-size:4rem">🚫</div><h2 style="color:var(--danger);margin:12px 0">Вы заблокированы</h2><p style="color:var(--text-secondary)">Причина: <strong>${esc(user.ban_reason||'нарушение правил')}</strong></p>${until?`<p style="color:var(--text-secondary)">До: ${until.toLocaleString('ru-RU')} (${dur})</p>`:'<p style="color:var(--text-secondary)">Навсегда</p>'}<p style="color:var(--text-secondary);margin:16px 0">Обязательно прочитайте правила. Кнопка появится через 3 минуты.</p><div class="rules-content"><h3 style="color:var(--danger)">🚫 ЗАПРЕЩЕНО:</h3><ul><li>Хейтинг и травля</li><li>Спам</li><li>Угрозы</li><li>Дискриминация</li><li>Контент 18+</li><li>Мошенничество</li><li>Чужая личность</li><li>Вредоносные ссылки</li></ul></div><p class="timer-text">Осталось: <strong id="banTimer">3:00</strong></p><button class="modal-btn" id="banConfirmBtn" disabled>Я понял(а)</button></div></div>`;
        let seconds = 180;
        ...
        btn.addEventListener('click', () => {
            // После прочтения правил и таймера пользователь может выйти, но остаётся забаненным
            // Ничего не делаем, просто возвращаемся к авторизации (он не может войти)
            app.innerHTML = `<div class="auth-container"><div class="auth-card"><p style="color:var(--text-secondary)">Вы заблокированы. Дождитесь окончания бана.</p></div></div>`;
        });
    };

    // Экран предупреждения (синий)
    const showWarningScreen = (warning) => {
        ...
        let seconds = 180;
        const timerEl = document.getElementById('warnTimer');
        const btn = document.getElementById('warnConfirmBtn');
        const interval = setInterval(() => { seconds--; const m=Math.floor(seconds/60),s=seconds%60; timerEl.textContent=`${m}:${s.toString().padStart(2,'0')}`; if(seconds<=0){clearInterval(interval);btn.disabled=false;timerEl.textContent='0:00';} },1000);
        btn.addEventListener('click', async () => {
            // Помечаем предупреждение как прочитанное
            await supabase.from('warnings').update({ is_read: true }).eq('id', warning.id);
            // Перенаправляем на страницу правил (с таймером уже прошёл, можно просто открыть приложение)
            renderApp();
        });
    };
    ...
    const setupComposer = () => {
        ...
        input.addEventListener('input', () => { const len = input.value.length; counter.textContent = `${len}/280`; postBtn.disabled = len === 0 && !pendingImage; });
        document.getElementById('attachImageBtn').addEventListener('click', () => document.getElementById('imageInput').click());
        document.getElementById('imageInput').addEventListener('change', (e) => { const file = e.target.files[0]; if (!file) return; pendingImage = file; const reader = new FileReader(); reader.onload = ev => { document.getElementById('pendingImagePreview').src = ev.target.result; document.getElementById('pendingImageContainer').classList.remove('hidden'); postBtn.disabled = false; }; reader.readAsDataURL(file); });
        document.getElementById('removePendingImage').addEventListener('click', () => { pendingImage = null; document.getElementById('pendingImageContainer').classList.add('hidden'); postBtn.disabled = input.value.length === 0; });
        postBtn.addEventListener('click', async () => {
            const content = input.value.trim();
            if (!content && !pendingImage) return;
            // Антиспам: проверяем последний такой же пост
            const { data: recent } = await supabase.from('chirps').select('created_at').eq('user_id', currentUser.id).eq('content', content).order('created_at', { ascending: false }).limit(1);
            if (recent && recent.length > 0 && (Date.now() - new Date(recent[0].created_at).getTime()) < 300000) {
                alert('Нельзя отправлять одинаковые посты чаще раза в 5 минут.');
                return;
            }
            let imageUrl = null;
            if (pendingImage) {
                const path = `chirps/${currentUser.id}_${Date.now()}.${pendingImage.name.split('.').pop()}`;
                await supabase.storage.from('images').upload(path, pendingImage);
                const { data } = supabase.storage.from('images').getPublicUrl(path);
                imageUrl = data.publicUrl;
            }
            const hashtags = content.match(/#\w+/g) || [];
            // Определяем огненный пост (streak_count >= 2)
            let isFire = false;
            const today = new Date().toISOString().split('T')[0];
            const lastDate = currentUser.last_post_date;
            let newStreak = currentUser.streak_count || 0;
            if (lastDate === today) {
                // уже постил сегодня, streak не меняем
            ...
            // Вставляем пост
            const { data: chirp } = await supabase.from('chirps').insert({
                user_id: currentUser.id, username: currentUser.username, avatar_emoji: currentUser.avatar_emoji || '👤',
                content, image_url: imageUrl, hashtags, is_fire: isFire
            }).select().single();
            // Обновляем streak и last_post_date
            ...
            // Обновляем тренды
            hashtags.forEach(async (tag) => { const { data: exist } = await supabase.from('trends').select('*').eq('hashtag', tag).single(); if (exist) { await supabase.from('trends').update({ count: exist.count + 1, updated_at: new Date() }).eq('id', exist.id); } else { await supabase.from('trends').insert({ hashtag: tag }); } });
            input.value = ''; pendingImage = null; document.getElementById('pendingImageContainer').classList.add('hidden'); postBtn.disabled = true; counter.textContent = '0/280';
            loadFeed();
        });
    };
    ...
    const loadFeed = async () => {
        ...
        if (activeTab === 'trends') { const { data: trends } = await supabase.from('trends').select('*').order('count', { ascending: false }).limit(20); container.innerHTML = trends?.map(t => `<div style="padding:12px 16px;border-bottom:1px solid var(--border);cursor:pointer" class="trend-item" data-tag="${t.hashtag}"><span style="font-weight:700">${t.hashtag}</span><br><span style="color:var(--text-secondary);font-size:0.85rem">${t.count} чириков</span></div>`).join('') || '<div style="text-align:center;padding:40px;color:var(--text-secondary)">Нет трендов</div>'; document.querySelectorAll('.trend-item').forEach(el => el.addEventListener('click', () => loadTrendFeed(el.dataset.tag))); return; }
    ...
    const renderChirp = (c) => `<div class="chirp-card" data-chirp-id="${c.id}" data-user-id="${c.user_id}"><div class="chirp-header"><div class="chirp-avatar">${c.avatar_emoji||'👤'}${c.is_verified?'<span class="verified-badge"><i class="fa-solid fa-check"></i></span>':''}</div><div><div class="chirp-author">${esc(c.username)} ${c.is_fire?'<span class="chirp-fire"><i class="fa-solid fa-fire"></i></span>':''}${c.is_masked?'<span style="color:#888">(маска)</span>':''}</div><div class="chirp-username">@${esc(c.username)} · ${formatTime(c.created_at)}</div></div></div><div class="chirp-content">${c.content.replace(/#\w+/g, '<span class="hashtag">$&</span>')}</div>${c.image_url?`<div class="chirp-image"><img src="${c.image_url}" onclick="window.open('${c.image_url}')"></div>`:''}<div class="chirp-actions"><button class="chirp-action like-btn" data-chirp-id="${c.id}"><i class="fa-heart fa-regular"></i> <span>${c.likes||0}</span></button><button class="chirp-action dislike-btn" data-chirp-id="${c.id}"><i class="fa-thumbs-down fa-regular"></i> <span>${c.dislikes||0}</span></button><button class="chirp-action rechirp-btn" data-chirp-id="${c.id}"><i class="fa-solid fa-retweet"></i> <span>${c.rechirps||0}</span></button><button class="chirp-action comment-btn" data-chirp-id="${c.id}"><i class="fa-regular fa-comment"></i></button></div></div>`;

    const setupChirpActions = () => {
        document.querySelectorAll('.chirp-card').forEach(card => {
            card.querySelector('.chirp-header')?.addEventListener('click', () => showProfile(card.dataset.userId));
        });
        document.querySelectorAll('.like-btn').forEach(b => b.addEventListener('click', async (e) => { e.stopPropagation(); const id = b.dataset.chirpId; const { data: exist } = await supabase.from('likes').select('*').eq('user_id', currentUser.id).eq('chirp_id', id).single(); if (exist) { await supabase.from('likes').delete().eq('id', exist.id); b.classList.remove('liked'); b.querySelector('i').className = 'fa-regular fa-heart'; } else { await supabase.from('likes').insert({ user_id: currentUser.id, chirp_id: id }); b.classList.add('liked'); b.querySelector('i').className = 'fa-solid fa-heart'; } const { count } = await supabase.from('likes').select('*', { count: 'exact' }).eq('chirp_id', id); b.querySelector('span').textContent = count || 0; await supabase.from('chirps').update({ likes: count }).eq('id', id); }));
        document.querySelectorAll('.dislike-btn').forEach(b => b.addEventListener('click', async (e) => { e.stopPropagation(); const id = b.dataset.chirpId; const { data: exist } = await supabase.from('dislikes').select('*').eq('user_id', currentUser.id).eq('chirp_id', id).single(); if (exist) { await supabase.from('dislikes').delete().eq('id', exist.id); b.classList.remove('disliked'); b.querySelector('i').className = 'fa-regular fa-thumbs-down'; } else { await supabase.from('dislikes').insert({ user_id: currentUser.id, chirp_id: id }); b.classList.add('disliked'); b.querySelector('i').className = 'fa-solid fa-thumbs-down'; } const { count } = await supabase.from('dislikes').select('*', { count: 'exact' }).eq('chirp_id', id); b.querySelector('span').textContent = count || 0; await supabase.from('chirps').update({ dislikes: count }).eq('id', id); }));
        document.querySelectorAll('.rechirp-btn').forEach(b => b.addEventListener('click', async (e) => { e.stopPropagation(); const id = b.dataset.chirpId; const { data: exist } = await supabase.from('rechirps').select('*').eq('user_id', currentUser.id).eq('chirp_id', id).single(); if (!exist) { await supabase.from('rechirps').insert({ user_id: currentUser.id, chirp_id: id }); const { count } = await supabase.from('rechirps').select('*', { count: 'exact' }).eq('chirp_id', id); b.querySelector('span').textContent = count || 0; await supabase.from('chirps').update({ rechirps: count }).eq('id', id); } }));
    ...
    const showComments = async (chirpId) => {
        ...
        overlay.innerHTML = `<div class="modal-card"><h3>💬 Комментарии</h3><div style="max-height:50vh;overflow-y:auto;margin-bottom:12px">${data?.map(c => `<div style="padding:8px 0;border-bottom:1px solid var(--border)"><strong>${esc(c.username)}</strong>: ${esc(c.content)}</div>`).join('') || '<p style="color:var(--text-secondary)">Нет комментариев</p>'}</div><input type="text" id="commentInput" class="modal-input" placeholder="Ваш комментарий..."><button class="modal-btn" id="sendCommentBtn">Отправить</button><button class="modal-btn secondary" id="closeCommentsBtn">Закрыть</button></div>`;
        document.body.appendChild(overlay);
        document.getElementById('closeCommentsBtn').addEventListener('click', () => overlay.remove());
        document.getElementById('sendCommentBtn').addEventListener('click', async () => { const content = document.getElementById('commentInput').value.trim(); if (!content) return; await supabase.from('comments').insert({ chirp_id: chirpId, user_id: currentUser.id, username: currentUser.username, content }); overlay.remove(); showComments(chirpId); });
    };
