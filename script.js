(function() {
    ...
    async function openProfile(userId) {
        ...
        if (feed) { feed.innerHTML = posts?.length ? '' : '<p style="color:#555;">Нет постов</p>'; posts?.forEach(post => { const c = createPostCard(post); if (c) feed.appendChild(c); }); }
    }
    ...
    document.getElementById('removeImagePreview').addEventListener('click', () => { selectedImage = null; imagePreview.classList.add('hidden'); updatePublishBtn(); });
    ...
    document.getElementById('removeVideoPreview').addEventListener('click', () => { selectedVideo = null; videoPreview.classList.add('hidden'); updatePublishBtn(); });
