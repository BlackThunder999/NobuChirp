const NobuChirp = (() => {
    ...
    const cleanup = () => {
        ...
        if (postTimerInterval) { clearInterval(postTimerInterval); postTimerInterval = null; }
    };
    ...
    const startPostCooldown = () => {
        ...
        postTimerInterval = setInterval(() => {
            remaining--;
            if (btnText) btnText.textContent = `${remaining}с`;
            if (remaining <= 0) {
                clearInterval(postTimerInterval);
                postTimerInterval = null;
                canPost = true;
                if (btn) btn.disabled = false;
                if (btnText) btnText.textContent = 'Чирикнуть';
                updatePostBtn();
            }
        }, 1000);
    };
    ...
    const updatePostBtn = () => {
        ...
        const btn = document.getElementById('postBtn');
        if (!btn) return;
        if (!canPost) { btn.disabled = true; return; }
    ...
    const setupComposer = () => {
        ...
        document.getElementById('imageInput')?.addEventListener('change', (e) => {
            ...
            reader.onload = ev => {
                ...
                const container = document.getElementById('pendingImageContainer');
        ...
        document.getElementById('removePendingImage')?.addEventListener('click', () => {
            pendingImage = null;
            const container = document.getElementById('pendingImageContainer');
        ...
        newPostBtn.addEventListener('click', async () => {
            if (!canPost || newPostBtn.disabled) return;
            ...
            try {
                ...
                await supabase.from('chirps').insert({
                    ...
                    avatar_emoji: freshUser?.avatar_emoji || currentUser.avatar_emoji || '👤',
                ...
                if (freshUser) {
                    currentUser.is_verified = freshUser.is_verified;
                    currentUser.avatar_emoji = freshUser.avatar_emoji;
                }

                hashtags.forEach(async (tag) => {
                    const { data: exist } = await supabase.from('trends').select('*').eq('hashtag', tag).single();
                    if (exist) {
                        await supabase.from('trends').update({ count: exist.count + 1 }).eq('id', exist.id);
                    } else {
                        await supabase.from('trends').insert({ hashtag: tag });
                    }
                });
    ...
    const loadFeed = async () => {
        ...
        if (activeTab === 'following') {
            ...
            if (ids.length === 0) {
                container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-secondary)">Подпишитесь на кого-нибудь</div>';
                return;
            }
            query = query.in('user_id', ids);
        }
        if (activeTab === 'trends') {
            const { data: trends } = await supabase.from('trends').select('*').order('count', { ascending: false }).limit(20);
            container.innerHTML = trends?.map(t => `<div class="trend-item" data-tag="${t.hashtag}"><span style="font-weight:700">${t.hashtag}</span><br><span style="color:var(--text-secondary);font-size:0.85rem">${t.count} чириков</span></div>`).join('') || '<div style="text-align:center;padding:40px;color:var(--text-secondary)">Нет трендов</div>';
            document.querySelectorAll('.trend-item').forEach(el => el.addEventListener('click', () => { activeTab = 'trends'; loadTrendFeed(el.dataset.tag); }));
            return;
        }
    ...
    const loadTrendFeed = async (tag) => {
        const { data } = await supabase.from('chirps').select('*').contains('hashtags', [tag]).order('created_at', { ascending: false });
        const container = document.getElementById('feedContainer');
        if (container) container.innerHTML = data?.map(c => renderChirp(c)).join('') || '<div style="text-align:center;padding:40px;color:var(--text-secondary)">Нет чириков</div>';
        setupChirpActions();
    };
    ...
    const setupChirpActions = () => {
        ...
        document.querySelectorAll('.like-btn').forEach(b => b.addEventListener('click', async (e) => {
            e.stopPropagation();
            const id = b.dataset.chirpId;
            const { data: exist } = await supabase.from('likes').select('*').eq('user_id', currentUser.id).eq('chirp_id', id).single();
            if (exist) { await supabase.from('likes').delete().eq('id', exist.id); }
            else { await supabase.from('likes').insert({ user_id: currentUser.id, chirp_id: id }); }
            const { count } = await supabase.from('likes').select('*', { count: 'exact' }).eq('chirp_id', id);
            b.querySelector('span').textContent = count || 0;
            b.classList.toggle('liked', !exist);
        ...
        document.querySelectorAll('.dislike-btn').forEach(b => b.addEventListener('click', async (e) => {
            e.stopPropagation();
            const id = b.dataset.chirpId;
            const { data: exist } = await supabase.from('dislikes').select('*').eq('user_id', currentUser.id).eq('chirp_id', id).single();
            if (exist) { await supabase.from('dislikes').delete().eq('id', exist.id); }
            else { await supabase.from('dislikes').insert({ user_id: currentUser.id, chirp_id: id }); }
            const { count } = await supabase.from('dislikes').select('*', { count: 'exact' }).eq('chirp_id', id);
            b.querySelector('span').textContent = count || 0;
            b.classList.toggle('disliked', !exist);
            b.querySelector('i').className = exist ? 'fa-regular fa-thumbs-down' : 'fa-solid fa-thumbs-down';
        }));
        document.querySelectorAll('.rechirp-btn').forEach(b => b.addEventListener('click', async (e) => {
            e.stopPropagation();
            const id = b.dataset.chirpId;
            const { data: exist } = await supabase.from('rechirps').select('*').eq('user_id', currentUser.id).eq('chirp_id', id).single();
            if (!exist) {
                await supabase.from('rechirps').insert({ user_id: currentUser.id, chirp_id: id });
                const { count } = await supabase.from('rechirps').select('*', { count: 'exact' }).eq('chirp_id', id);
                b.querySelector('span').textContent = count || 0;
            }
        }));
        document.querySelectorAll('.comment-btn').forEach(b => b.addEventListener('click', (e) => {
            e.stopPropagation();
            showComments(b.dataset.chirpId);
        }));
    };

    // ========== КОММЕНТАРИИ, ПРОФИЛЬ, АДМИНКА (без изменений, как в предыдущей версии) ==========
    ...
    const init = async () => {
        ...
        if (saved) {
            try {
                ...
                if (user) {
                    currentUser = user;
                    if (await checkWarningsAndBan(user)) return;
                    renderApp();
                } else {
                    renderAuth();
                }
